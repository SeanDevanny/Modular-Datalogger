...
async function Chartbuilder(object){
    ...
    for(let i=0; i<object.length; i++){
        refinedobject.series[i].name = object[i].Name;
        for(let j=0; j<object[i].Values.length; j++){
            refinedobject.series[i].points[j] = {x: `${object[i].Values[j]}`, y: `${object[i].TimeDifference[j]}`}
        }
    }
